...
CREATE TABLE `reservering` (
  ...
  `accommodatie_code` varchar(5) DEFAULT NULL,
  `reiziger_code` varchar(45) NOT NULL,
  ...
  CONSTRAINT `accommodatie_code` FOREIGN KEY (`accommodatie_code`) REFERENCES `accommodatie` (`accommodatie_code`) ON DELETE SET NULL,
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `verwijderAccommodatie`(IN acc_code VARCHAR(5))
BEGIN
      IF ((SELECT accommodatie_code FROM reservering WHERE betaald = 1 AND vertrekdatum < CURRENT_DATE) = acc_code)
      THEN
      DELETE FROM accommodatie WHERE accommodatie_code = acc_code;
      END IF;
    END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50001 VIEW `boekingsoverzicht` AS select `acc`.`naam` AS `naam`,`acc`.`stad` AS `stad`,`acc`.`land` AS `land`,`acc`.`soort` AS `soort`,concat(`reiz`.`voornaam`,' ',`reiz`.`achternaam`) AS `reiziger`,`reiz`.`reiziger_code` AS `reiziger_code`,`res`.`aankomstdatum` AS `aankomstdatum`,`res`.`vertrekdatum` AS `vertrekdatum`,(to_days(`res`.`vertrekdatum`) - to_days(`res`.`aankomstdatum`)) AS `aantaldagen`,`res`.`betaald` AS `betaald` from ((`reservering` `res` join `accommodatie` `acc` on((`acc`.`accommodatie_code` = `res`.`accommodatie_code`))) join `reiziger` `reiz` on((`reiz`.`reiziger_code` = `res`.`reiziger_code`))) order by `res`.`aankomstdatum` */;
...
-- Dump completed on 2022-05-23 16:44:10
